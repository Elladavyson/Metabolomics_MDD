...
write.table(spectrometer, 'Metabolomics_Aut2021/spectrometer_covariate.tsv', sep = '\t', row.names = F, quote = F)
write.table(nmr_withqc_noallNA, 'Metabolomics_Aut2021/NMR_baseline_withqc.tsv', sep = '\t', row.names = F, quote = F)



#merge with the baseline file currently used in the analysis for the CIDI phenotype 

cidi_pheno <- read.csv('Metabolomics_Aut2021/cidi_pheno/norm_metabol_allmerged.csv', header = TRUE)

cidi_spectro_merged <- merge(cidi_pheno, nmr_withqc_noallNA[,c('f.eid', 'spectrometer')], by = 'f.eid')
write.csv(cidi_spectro_merged, 'Metabolomics_Aut2021/norm_metabol_allmerged_wispectro.csv', row.names = F)